...
function exerciceSuccess() {
    ...
    successText.innerHTML = "<i>Félicitations ! Hubert à récupérer tous son équipement et même un snack !<br><br> On peut maintenant partir à l'aventure sans aucun soucis !!<br><br><img class='illustration' src='../img/hubert_snack.webp'></img>"
}
    ...
    btn2.onclick = function() {
        if (select != null) {
            if (select.value === "Hubert" && style.borderRadius != "3px") {
                ...
                exerciceFailure("Ici la selection s'affiche mais on ne peut pas lire ni voir ce qu'on a sélectionner, essayez autre chose !", "ie2");
            } else if (style.display == "none") {
                exerciceFailure("Comme dans l'exercice 2, le display empêche de pouvoir voir le selecteur, essayez autre chose !", "ie2");
            } else {
                exerciceFailure("Il semblerait que vous n'ayez pas selectionner la bonne valeur, essayez autre chose", "ie2");
            }
            ...
            exerciceFailure("Hubert ne sais pas ce qu'il s'est passé, avec-vous décommentez l'une des deux options de l'html ?", "ie2");
        }
    }
}
